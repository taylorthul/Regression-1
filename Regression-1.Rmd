...
*If you run the code data () it will run the data of all the data sets loaded in memory to just give a quick summary.
...
```{r}
#graph child on parent for testing dataset, that squiggle is called tilda. anything in front of the tilda is the outcome and after is the predictorys so makes child y and parent x, so basically y~x. this is a 'formula' or function. wanting to look at how much the parent's height predicts the child's height. we used the test data because there were less points and less over plotting
...
# lm is a workhorse in R -> go to the help 
# if we just ran the lm by itself and didn't assign it to anything we would just get the output like the intersept, slope, t test, p values 
# but we don't want to keep having to do it over and over again so we assign it to an object 
reg1 <- lm(child ~ parent, data = Galton.train)
#must run summary command to get output
summary(reg1)
#creates list that has all kinds of stuff in there so the summary lets us read it like a human
```
...
#set up the 0 lin easier this code sets a line with the slope and intersept 
abline(0,0)
...
summary(reg1)
```
*so we can reject the hypothesis that beta equals 0
...
# can change type to "html" which will look very nice once everything is knit together but is a mess now 
...
#if you have the others in what is the effect of education? and the other two independent variables? 
```
